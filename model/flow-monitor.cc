...
//aghax
#define DEFAULT_PERCENTILE 0.99
//aghax
  ...
  if (iter == m_flowStats.end ())
    {
      ...
      ref.averageLatency = Seconds (0);
      ref.percentileLatency = Seconds (0);
...
FlowMonitor::ReportLastRx (Ptr<FlowProbe> probe, uint32_t flowId, uint32_t packetId, uint32_t packetSize)
{
  ...
  //aghax
  stats.delays_container.push_back(delay);

  int index = std::ceil(stats.delays_container.size() * percentile) - 1;
  stats.percentileLatency = stats.delays_container.at(index);
  
  if(stats.rxPackets)
    stats.averageLatency = stats.delaySum/stats.delays_container.size();
    
  // throughput
  stats.throughput = (stats.rxBytes * 8.0)/(stats.timeLastRxPacket.GetSeconds()-
                      stats.timeFirstTxPacket.GetSeconds()) / 1024.0;
  //aghax
...
#define ATTRIB(name) << " " # name "=\"" << flowI->second.name << "\""
      ...
      ATTRIB (percentileLatency)
      ATTRIB (averageLatency)
      ATTRIB (throughput)
      << ">\n";
#undef ATTRIB
